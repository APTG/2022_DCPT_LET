...
| **TOPAS IEC/BEV** | `DCM_to_IEC` `RotX=90`; beam along world **Y**; scoring `RotZ 180` | TOPAS only |
...
the +z side). TOPAS is on a third frame that is closest to the TPS itself.
...
on whether a code stores the beam as +Z, −Z, or along Y. This is why the existing 1D
comparisons are valid across codes today and remain valid under this contract. A
code's readback must produce depth in this sense — if its native axis runs the other
way (e.g. TOPAS's reversed Y), the declared transform reverses it; it must **not** be
hand-reversed with an undocumented `np.flip`.
...
| TOPAS | IEC/BEV (beam along world Y, `RotZ 180`) | map world **Y → depth** with reversal; lateral axes per `DCM_to_IEC` (`RotX=90`) — the transform is the inverse of the DICOM→IEC rotation the export already applies |

**TOPAS beam generation.** The TOPAS beam input is generated by dicomexport
(`--export-fmt topas`), driven by [`tools/make_topas.sh`](../tools/make_topas.sh),
which requires **dicomexport ≥ 1.4.4**. It is exported with **`--nozzle-side neg-z`**,
the verified patient-correct IEC convention (source on the anterior side, beam
...
> **Note:** these transforms are declarative in the current pass — the plotters do not
> yet consume them for automatic 2D/3D remapping (that is deferred, see below). The
> readback is applied once, in TOPAS post-processing
> ([`data/topas/postprocess_local.py`](../data/topas/postprocess_local.py)), which maps
> the Y-binned TOPAS output onto the shared isocenter-referenced depth grid — instead of
> scattered bare `np.flip` calls.
