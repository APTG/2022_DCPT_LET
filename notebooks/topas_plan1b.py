...
files = []
Dose = BinnedResult('data/topas/results/output/plan01_geoB_SOBP95/Scoring_protonD_YBox.csv')
LET = BinnedResult('data/topas/results/output/plan01_geoB_SOBP95/Scoring_protonLET_YBox.csv')
...
ax1.fill_between(x=[0, 15], y1=0, y2=max(np.squeeze(Dose.data['Sum']))*scale+1, facecolor='blue', alpha=0.1)
ax1.fill_between(x=[15.0, 15.5], y1=0, y2=max(np.squeeze(Dose.data['Sum']))*scale+1, facecolor='black', alpha=0.3)
ax1.fill_between(x=[15.5, 20.], y1=0, y2=max(np.squeeze(Dose.data['Sum']))*scale+1, facecolor='blue', alpha=0.1)

# plt.vlines(x =[11.8, 12.8], ymin = 0, ymax =max(np.squeeze(dose.data['Sum'])), color = 'red', label = 'Cellflask 3')
# plt.show()
plt.savefig("data/topas/results/plan01_geoB_SOBP95/" + Dose.quantity + "_" + LET.quantity)
