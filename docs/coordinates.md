...
**TOPAS setup.** The beam input is generated by dicomexport (`--export-fmt topas`,
driven by [`tools/make_topas.sh`](../tools/make_topas.sh), **dicomexport ≥ 1.4.4**),
exported with **`--nozzle-side neg-z`** (the verified patient-correct IEC convention;
never `pos-z` for patient runs). The RTPLAN export is *beam-only* and is shared once
per plan+field+model. The **BEV frame** (`World→Couch→DCM_to_IEC→Gantry`) is taken
verbatim from dicomexport 1.4.4 into
[`data/topas/input/topas_frame.txt`](../data/topas/input/topas_frame.txt); each
`main.txt` includes it and **parents the slab phantom + all scoring to `Gantry`**, so
everything is scored in BEV with depth = +Z. Slab `TransZ` come from
